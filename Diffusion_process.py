...
class DiffusionProcess(nn.Module):
    ...
    def get_output(self, model, emb_s, reweight=False):
        ...
        # 原错误代码：noise = th.randn_like(emb_s)
    # 新代码：针对稀疏张量生成噪声（仅非零元素添加噪声）
        if emb_s.is_sparse:
            emb_s = emb_s.coalesce()
            # 1. 提取稀疏张量的非零元素值、 indices（位置）和形状
            values = emb_s.values()  # 非零元素的值（密集张量）
            indices = emb_s.indices()  # 非零元素的位置（2xN 张量）
            size = emb_s.size()        # 稀疏张量的整体形状
            
            # 2. 仅对非零元素生成正态分布噪声（与 values 同形状）
            noise_values = th.randn_like(values, device=values.device)
            
            # 3. 重构稀疏噪声张量（保持与 emb_s 相同的稀疏结构）
            noise = th.sparse.FloatTensor(indices, noise_values, size).to(emb_s.device)
        else:
            # 若为密集张量，按原逻辑生成噪声（兼容其他场景）
            noise = th.randn_like(emb_s)
